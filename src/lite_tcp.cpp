...
LiteTcpServer::LiteTcpServer(const std::string &server_ip, int port,
  ...
  // keep alive
  int keep_alive = 1;
  if (setsockopt(sockfd_, SOL_SOCKET, SO_REUSEADDR, &keep_alive,
                 sizeof(keep_alive)) < 0) {
  ...
  // reuse address
  int reuse_address = 1;
  if (setsockopt(sockfd_, SOL_SOCKET, SO_REUSEADDR, &reuse_address,
  ...
  // reuse port
  int reuse_port = 1;
  if (setsockopt(sockfd_, SOL_SOCKET, SO_REUSEPORT, &reuse_port,
                 sizeof(reuse_port)) < 0) {
    perror("setsockopt reuse_address");
    exit(EXIT_FAILURE);
  }
...
bool LiteTcpServer::run() {
  ...
  process_thread_ = std::thread([&] {
    ...
    ev.events =
        EPOLLIN |
        EPOLLET; // The associated file is available for read operations.
    ev.data.fd = sockfd_;
    ...
    while (run_) {
      ...
      for (int i = 0; i < nfds; ++i) {
        if (events[i].data.fd == sockfd_) {
          ...
          // nonblock
          int conn_fd = accept4(sockfd_, reinterpret_cast<sockaddr *>(&cli),
          ...
          std::cout << "connected to client, " << inet_ntoa(cli.sin_addr) << ":"
...
// Client
LiteTcpClient::LiteTcpClient(const std::string &server_ip, int server_port,
                             const RecvCallback &func)
    : sockfd_(-1), run_(false), stop_(false), callback_(func) {
  sockfd_ = socket(AF_INET, SOCK_STREAM, 0);
  struct sockaddr_in cli;
